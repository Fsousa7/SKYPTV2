...
class DatabaseHelper {
  ...
  Future<Database> _initDB(String filePath) async {
    ...
    return await openDatabase(
      path, 
      version: 2,  // Incrementado para versão 2 
      onCreate: _createDB,
      onUpgrade: _onUpgrade, // Adicionado manipulador de atualização
    );
  }

  // Função para atualizar o banco de dados quando a versão muda
  Future _onUpgrade(Database db, int oldVersion, int newVersion) async {
    if (oldVersion < 2) {
      // Adiciona a nova coluna para filamentos múltiplos
      try {
        await db.execute('ALTER TABLE modelo3d ADD COLUMN filamentos_usados TEXT');
        print("Base de dados atualizada para suportar múltiplos filamentos");
      } catch (e) {
...
''');
  ...
  Future<int> insertModelo(Map<String, dynamic> data, int userId) async {
    ...
    try {
      // Verificar se estamos no novo formato com múltiplos filamentos
      if (data.containsKey('filamentos_usados')) {
        // Pegar o primeiro filamento para compatibilidade com o esquema antigo
        List<dynamic> filamentosUsados = [];
        try {
          if (data['filamentos_usados'] is String) {
            filamentosUsados = jsonDecode(data['filamentos_usados']);
          } else {
            // Se já for uma lista, converta para string
          ...
          print("Erro ao processar filamentos: $e");
          data['filamento_id'] = 0;
          data['tipo_filamento'] = '';
          data['gramas_utilizadas'] = 0;
        }
      }
      ...
      // Fallback para formato compatível se der erro
      ...
      try {
        // Processar filamentos para formato antigo
        if (data.containsKey('filamentos_usados')) {
          List<dynamic> filamentosUsados = [];
          try {
            if (data['filamentos_usados'] is String) {
            ...
            print("Erro ao processar filamentos (fallback): $e");
          }
        }
